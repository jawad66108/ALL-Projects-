...
void BFS(int start, int goal) {
    bool visited[N] = {false}; // keeps track of cities already visited
    int parent[N];  // stores from which city we came
    ...
    cout << "  " << string(60, '-') << "\n";
    int path[50], count = 0, temp = goal;
    while (temp != -1) {
        path[count++] = temp;
        temp = parent[temp];  //Move one step backward in the path
    }
    
    if (parent[goal] == -1) { //If goal has no parent
     
        cout << "  No path found!\n";
    } else {
        cout << "  Route: ";
        for (int i = count - 1; i >= 0; i--) { //Reverse the path [start -> goal]
            cout << cities[path[i]];
            if (i != 0) cout << " -> ";
        }
        cout << "\n  Total Stops: " << (count - 1) << "\n";
...
bool dfsFound = false;  //true when the goal city is found
int dfsParent[N];

void DFSUtil(int node, int goal, bool visited[]) {
    visited[node] = true;
...
void DFS(int start, int goal) {
    ...
    for (int i = 0; i < N; i++) dfsParent[i] = -1;
    dfsFound = false;
...
void saveResultToJSON(
    ...
    ofstream file("result.json", ios::trunc); // force overwrite
    if (!file.is_open()) {
        cout << "Error: result.json could not be created\n";
        return;
    }

    file << "{\n";
    file << "  \"algorithm\": \"" << algorithm << "\",\n";
    file << "  \"source\": \"" << cities[source] << "\",\n";
    file << "  \"destination\": \"" << cities[destination] << "\",\n";
    file << "  \"path\": \"" << path << "\",\n";
    file << "  \"cost\": " << cost << "\n";
    file << "}\n";
